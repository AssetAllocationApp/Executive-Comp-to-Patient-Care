    ...
    for event, elem in iterparse(xml_path, events=("start", "end")):
        ...
        if event == "start":
            ...
            for wanted_path in list(remaining):
                if (
                    len(stack) >= len(wanted_path)
                    and tuple(stack[-len(wanted_path) :]) == wanted_path
                ):
...
def parse_schedule_j_person_rows(xml_path: str) -> List[Dict]:
    ...
    groups += root.findall(
        ".//irs:IRS990ScheduleJ/irs:RltdOrgOfficerTrstKeyEmplGrp", IRS_NS
    )
    ...
    for g in groups:
        ...
        row = {
            ...
            # Base
            "BaseComp_Filer": _text_to_int(
                get("BaseCompensationFilingOrgAmt").text
                if get("BaseCompensationFilingOrgAmt") is not None
                else None
            ),
            "BaseComp_Related": _text_to_int(
                get("CompensationBasedOnRltdOrgsAmt").text
                if get("CompensationBasedOnRltdOrgsAmt") is not None
                else None
            ),
            # Bonus (covers “bonus & incentive” in IRS parlance)
            "Bonus_Filer": _text_to_int(
                get("BonusFilingOrganizationAmount").text
                if get("BonusFilingOrganizationAmount") is not None
                else None
            ),
            "Bonus_Related": _text_to_int(
                get("BonusRelatedOrganizationsAmt").text
                if get("BonusRelatedOrganizationsAmt") is not None
                else None
            ),
            # Other comp
            "Other_Filer": _text_to_int(
                get("OtherCompensationFilingOrgAmt").text
                if get("OtherCompensationFilingOrgAmt") is not None
                else None
            ),
            "Other_Related": _text_to_int(
                get("OtherCompensationRltdOrgsAmt").text
                if get("OtherCompensationRltdOrgsAmt") is not None
                else None
            ),
            # Deferred
            "Deferred_Filer": _text_to_int(
                get("DeferredCompensationFlngOrgAmt").text
                if get("DeferredCompensationFlngOrgAmt") is not None
                else None
            ),
            "Deferred_Related": _text_to_int(
                get("DeferredCompRltdOrgsAmt").text
                if get("DeferredCompRltdOrgsAmt") is not None
                else None
            ),
            # Nontaxable benefits
            "Nontax_Filer": _text_to_int(
                get("NontaxableBenefitsFilingOrgAmt").text
                if get("NontaxableBenefitsFilingOrgAmt") is not None
                else None
            ),
            "Nontax_Related": _text_to_int(
                get("NontaxableBenefitsRltdOrgsAmt").text
                if get("NontaxableBenefitsRltdOrgsAmt") is not None
                else None
            ),
            # Totals (preferred if present)
            "TotalComp_Filer": _text_to_int(
                get("TotalCompensationFilingOrgAmt").text
                if get("TotalCompensationFilingOrgAmt") is not None
                else None
            ),
            "TotalComp_Related": _text_to_int(
                get("TotalCompensationRltdOrgsAmt").text
                if get("TotalCompensationRltdOrgsAmt") is not None
                else None
            ),
        }
        ...
        if total_reported > 0:
            ...
            row["TotalComp_AllIn"] = (
                row["BaseComp_Filer"]
                + row["BaseComp_Related"]
                + row["Bonus_Filer"]
                + row["Bonus_Related"]
                + row["Other_Filer"]
                + row["Other_Related"]
                + row["Deferred_Filer"]
                + row["Deferred_Related"]
                + row["Nontax_Filer"]
                + row["Nontax_Related"]
            )
...
def parse_xml_dir(dir_path, save_path, wanted, workers=os.cpu_count(), limit=None):
        ...
        for idx in os.listdir(os.path.join(src_dir, year)):
            for folder in os.listdir(os.path.join(src_dir, year, idx)):
                ...
                save_path = os.path.join(
                    dest_dir, f"{year}_{idx}_{folder}_processed.csv"
                )
...
def clean_990(df_990, year, min_rev, max_rev):
    for col in ["revenue", "expenses", "TotalComp_AllIn", "Bonus_Total"]:
        if col in df_990.columns:
            df_990[col] = (
                pd.to_numeric(df_990[col], errors="coerce").fillna(0).astype(float)
            )
    df_990 = df_990[df_990["tax_year"] == year]
    df_990 = (
        df_990.groupby(["name", "ein", "tax_year", "state", "city"], dropna=False)
    ...
    df_990["incentive_perc"] = df_990.apply(
        lambda r: (
            r["Bonus_Total"] / r["TotalComp_AllIn"] if r["TotalComp_AllIn"] > 0 else 0
        ),
        axis=1,
    )
    df_990["op_margin"] = df_990.apply(
        lambda r: (
            (r["revenue"] - r["expenses"]) / r["revenue"] if r["revenue"] > 0 else 0
        ),
        axis=1,
    )
...
def get_comp_mort(cms_root_path):
    comp_mort = pd.read_csv(
        os.path.join(cms_root_path, "Complications_and_Deaths-Hospital.csv")
    )
    comp_mort = comp_mort.pivot_table(
        index=["Facility ID", "Facility Name", "City", "State", "ZIP Code"],
        columns=["Measure Name"],
        values=["Denominator", "Score", "Lower Estimate", "Higher Estimate"],
        aggfunc="first",
    ).reset_index()
    comp_mort.columns = [
        f"{measure}: {val}" if len(measure) > 0 else val
    ...
    comp_mort = comp_mort.drop(
        columns=[
            ...
            if (comp_mort[c] == "Not Applicable").all()
            or (comp_mort[c] == "Not Available").all()
        ]
    )
...
def get_hosp_gen_info(cms_root_path):
    hosp_info = pd.read_csv(
        os.path.join(cms_root_path, "Hospital_General_Information.csv")
    )
    return hosp_info


def get_hvbp_clinical_outcomes(cms_root_path):
    hvbp_clin_out = pd.read_csv(
        os.path.join(cms_root_path, "hvbp_clinical_outcomes.csv")
    )
...
def get_healthcare_system():
    if not os.path.exists("./data/health_system.csv"):
        print("Could not find ./data/health_system.csv")
        print(
            "go to this url:\n\n"
            "https://www.ahrq.gov/chsp/data-resources/compendium-2022.html\n\n"
            "Look under the heading called '2022 Compendium Files'\n"
            "Then under 'Hospital Linkage File',\n"
            "Download that csv, reame it to 'health_system.csv',"
            "and place it in the ./data/ directory for this project"
        )
...
def join_datasets(
    ...
    match opts.get("region_pivot", "None"):
        case "Region":
            ...
            hosp_info = _get_dummies_and_record(
                hosp_info, cols=["Region"], drop_first=True, dtype=int
            )
        case "Division":
    ...
    if opts.get("healthcare_system_pivot"):
        ...
        sys_df["health_sys_id"] = sys_df["health_sys_id"].fillna(
            "Not in a Hospital System"
        )
        ...
        if small_cutoff:
            ...
            sys_df["health_sys_id"] = sys_df["health_sys_id"].apply(
                lambda x: (
                    ...
                    if sys_df_vals.get(x, 0) < small_cutoff
                    else x
                )
            )
            sys_df = sys_df[["ccn", "health_sys_id"]].copy()
            sys_df = _get_dummies_and_record(
                sys_df, cols=["health_sys_id"], drop_first=True, dtype=int
            )
        else:
    ...
    cost_report["is_teaching_hospital"] = (
    ...
    if missing:
        ...
        print(
            f"Warning: requested columns missing from merged dataframe and will be skipped: {sorted(list(missing))}"
        )
